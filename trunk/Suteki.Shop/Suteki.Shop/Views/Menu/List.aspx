...
<%= Html.Grid(Model.Menu.Contents.InOrder()).Columns(column => {
		...
		column.For(x => Html.Partial("UpDown")).HeaderAttributes(@class => "thin"); //Re-ordering arrows
		column.For(x => x.IsMenu ? Html.ActionLink<CmsController>(c => c.Add(x.Id), "New Page").ToString() : "&nbsp;").DoNotEncode().Named("&nbsp;").HeaderAttributes(@class => "thin");
        column.For(x => x.IsMenu ? Html.ActionLink<MenuController>(c => c.New(x.Id), "New Menu").ToString() : "&nbsp;").DoNotEncode().Named("&nbsp;").HeaderAttributes(@class => "thin");
