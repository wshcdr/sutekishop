<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Admin.Master" Inherits="Suteki.Shop.ViewPage<ScaffoldViewData<Suteki.Shop.PostZone>>" %>
...
<div class="admin-form">
	...
	<%= Html.Grid(Model.Items).Columns(column => {
		...
		column.For(x => Html.UpArrowLink<PostZoneController>(c => c.MoveUp(x.Position, 1))).DoNotEncode().Named("&nbsp;").HeaderAttributes(@class => "arrow");
		column.For(x => Html.DownArrowLink<PostZoneController>(c => c.MoveDown(x.Position, 1))).DoNotEncode().Named("&nbsp;").HeaderAttributes(@class => "arrow");
   }) %>
